...
export default function CourseForm() {
    ...
    const [curriculums, setCurriculums] = useState(() => {
        ...
        const titleLesson = 'Lecture 1';
        const slug = slugify(`${title}-${nanoid(6)}`, { lower: true });

        // setCourse({ ...course, curriculumsSlug: course.curriculumsSlug.push(slug) });

        const slugLesson = slugify(`${titleLesson}-${nanoid(10)}`, { lower: true });
        const initialCurriculums = [
            {
                title,
                lessons: [{ title: titleLesson, slug: slugLesson }],
                lessonsSlug: [slugLesson],
                slug,
            },
        ];
        const storagedCurriculums = JSON.parse(localStorage.getItem('curriculums'));
    ...
    const handleClick = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post('/course', { objs: [course, curriculums] });
    ...
    const handleAddSection = () => {
        setCurriculums((prev) => {
            ...
            // setCourse({ ...course, curriculumsSlug: course.curriculumsSlug.push(newCurriculum.slug) });
    ...
    return (
        <div className="container-fluid">
            <div className="row">
                ...
                <div className="col-sm-4">
                    <div className="mb-3 form-control">
                        <FileHtmlProcess course={course} setCourse={setCourse}></FileHtmlProcess>
                    </div>
                </div>
            </div>
